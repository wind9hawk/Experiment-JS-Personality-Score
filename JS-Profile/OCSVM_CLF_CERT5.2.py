# coding:utf-8
# 针对CERT5.2数据集，基于新的26JS用户满意度特征，训练OCSVM分类用户满意度；
# 与上个OCSVM的不同
# -1. 考虑了用户18个月是否全部在职，训练集用户必须18个月均在职；
# -2. 在18个月均在职的用户中，从2000个用户中选择1600个用户用于训练OCSVM；（训练比例0.8）
# -3. 剩余的18个月均在职的用户+18个月不全在职的用户+18个月全在职的Insiders构成测试集；
# 首先从CERT中读取用户特征
# PCA降维；
# 归一化Normalizer
...
print '读入新的用户JS特征数据...\n'
f_cert = open(r'CERT5.2-2009-12-New-25JS.csv', 'r')
f_cert_lst = f_cert.readlines()
f_cert.close()
print '读取用户工作状态数据...\n'
f_jobs = open(r'CERT5.2-2009-12-JobState.csv', 'r')
f_jobs_lst = f_jobs.readlines()
f_jobs.close()
print '读入CERT5.2中的Insiders用户...\n'
f_insiders = open(r'CERT5.2-2009-12-Insiders.csv', 'r')
f_insiders_lst = f_insiders.readlines()
f_insiders.close()
print 'JS特征文件、工作状态文件与Insiders文件均读入完毕...\n'

print '开始从2000个用户中选择训练集...\n'
# 本脚本实验从行索引的角度计算
...
# 1761个用户干满18个月
# 238个用户未干满18个月
# 总计1999个用户，少了一个主席用户（原始LDAP长度小于9）
# 需要从1663个用户中随机抽取63个补充到测试集，比例为0.037（约等于0.04）
# 考虑标题行在内
# 如果考虑上述去掉标题行的索引，应当-2T5TGUI12QWER
# 使用索引作为元素指标时，一定要注意先后两个列表中元素的索引未必一致，因此不可直接拿原先的索引套用新列表

print '从全职用户集合中去掉Insiders到测试集...\n'
# 已经有了Insiders文件，从中读取得到了f_insiders_lst
Insiders_nm = []
# Insiders_nm = ['ACM2278', 'CMP2946', 'PLJ1771', 'CDE1846', 'MBG3183']
for user in f_insiders_lst:
    user_l = user.strip('\n').strip(',').split(',')
    # 字符串split(',')后得到一个字符串列表，只有选择[0]才代表一个字符串字段，否则是整个列表类型
    # print 'Insider is ', user_l, '\n'
    Insiders_nm.append(user_l[0])
print 'Insiders列表准备完毕...\n'

# print Insiders_nm[0:10], '\n'
# sys.exit()

print '从全职用户中剔除Insiders...\n'
Cnt_Insider_AllJobs = 0.0
for user_index in Users_All_Jobs_index:
    # print Users_nm[user_index], '\n'
    if Users_nm[user_index] in Insiders_nm:
        ...
        Cnt_Insider_AllJobs += 1
        continue
    else:
        continue
print '去掉Insiders后全职用户还有...', len(Users_All_Jobs_index), '\n'
print Cnt_Insider_AllJobs, '\n'
# 出去Insiders后，全职用户还剩下1732，去掉了29个Insiders
# 此时从1732中选择0.076约为132个用户补充到测试集
# sys.exit()
...
X_train, X_test_0, Y_train_0, Y_test_0 = train_test_split(Users_All_Jobs_index, Y_train, test_size=0.076, random_state=15)
# 原来全工的用户的一部分补充到测试集
...
def LocateIndex(user_nm, f_lst):
    ...
    while i < len(f_lst):
        if f_lst[i] == user_nm:
            ...
            continue
    return -10000  # 表示未找到
print Users_26JS[0:2], '\n'
print Users_nm[0:10], '\n'
# sys.exit()
# 对于CERT5.2的99个Insiders而言，其中：
# 场景一为29个；
# 场景二为30个；
# 场景三为10个；
# 场景四为30个：
...
Cnt_Positive_4 = 0.0
Insiders_Positive = []
for insider in Insiders_nm:
    line_lst = Users_26JS_pca_nor[LocateIndex(insider, Users_nm)]
    if clf.predict(line_lst) == -1:
        if Insiders_nm.index(insider) < 29:
            Cnt_Positive_1 += 1
        if Insiders_nm.index(insider) > 28 and Insiders_nm.index(insider) < 59:
            Cnt_Positive_2 += 1
        if Insiders_nm.index(insider) > 58 and Insiders_nm.index(insider) < 69:
            Cnt_Positive_3 += 1
        if Insiders_nm.index(insider) > 68:
            Cnt_Positive_4 += 1
        Insiders_Positive.append(insider)
    else:
        continue
Cnt_Positive = Cnt_Positive_1 + Cnt_Positive_2 + Cnt_Positive_3 + Cnt_Positive_4
# print 'Insiders中识别为正类的有：', Cnt_Positive, '\n'
print '场景1召回率为： ', Cnt_Positive_1, '\t',Cnt_Positive_1/29, '\n'
print '场景2召回率为： ', Cnt_Positive_2, '\t',  Cnt_Positive_2/30, '\n'
print '场景3召回率为： ', Cnt_Positive_3, '\t', Cnt_Positive_3/10, '\n'
print '场景4召回率为： ', Cnt_Positive_4, '\t', Cnt_Positive_4/30, '\n'
print '召回率为： ', Cnt_Positive/len(Insiders_nm), '\n'
