...
def JS_4(center):
    ...
    return score
def JS_5(center):
    score = center[2] * (1 - math.log10(center[0] + center[1] + 10))
...
for line in Users_26JS_scale:
    ...
    while i < len(line_lst):
        ...
        if i < 5:
            ...
            i += 1
            continue
        if i > 4 and i < 13: # < 13 4 + 4 = 8，未考虑下标14的团队工作时间情况
            if i == 5 or i == 6 or i == 11 or i == 12: # 先仅考虑均值的CPB
                ...
                i += 1
                continue
        # if i > 13 and i < 20:  <23# 计算部门影响时暂时不考虑中位数的作用
        if i > 13 and i < 22:
            if i == 14 or i == 15 or i == 20 or i == 21:
...
for user in Users_5JS:
    ...
    # while k < len(user):
        ...
        # k += 1
    if user.count(1) < K_value: #or score_tmp <= Risk_median:
...
f_5JS = open(r'CERT4.2-2009-12-5JS-Label.csv', 'r')
...
Recall_3 = 0.0
Recall_all = 0.0
for line in f_5JS_lst:
    line_lst = line.strip('\n').strip(',').split(',')
    Insiders_1 = f_insiders_lst[0].strip('\n').strip(',').split(',')
    Insiders_2 = f_insiders_lst[1].strip('\n').strip(',').split(',')
    Insiders_3 = f_insiders_lst[2].strip('\n').strip(',').split(',')
    # print Insiders_1, '\n'
    # print 'AJR0932' in Insiders_1, '\n'
    #if line_lst[0] in Insiders_1:
    #    # print line_lst[0], '\n'
    #    print line_lst, line_lst[5], line_lst[6], '\n'
    #    sys.exit()
        #if float(line_lst[6]) < K_value:
        #    # print line_lst[0], line_lst[6], '\n'
        #    Recall_1 += 1
         #   Recall_all += 1
        #    continue
    #if line_lst[0] in f_insiders_lst[1]:
    #    if float(line_lst[6]) < K_value:
    #        Recall_2 += 1
    #        Recall_all += 1
     #       continue
    #if line_lst[0] in f_insiders_lst[2]:
    #    if float(line_lst[6]) < K_value:
    #        Recall_3 += 1
    #        Recall_all += 1
     #       continue
print Recall_1, len(Insiders_1) - 1, '\n'
print 'Recall is : ', Recall_1 / 30, Recall_2 / (len(Insiders_2) - 1), Recall_3 / (len(Insiders_3) - 1), '\n'
print 'Recall all is ', Recall_all / (len(Insiders_1) + len(Insiders_2) + len(Insiders_3) - 3)
